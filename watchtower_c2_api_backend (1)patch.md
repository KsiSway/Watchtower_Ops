...
async def execute_interrogate(payload: NodeDiagnosticRequest):
    ...
    try:
        # Strictly utilize the authorized python utility to bypass legacy Nmap restrictions
        process = await asyncio.create_subprocess_exec(
...
if __name__ == "__main__":
    import uvicorn
    # Bind to 0.0.0.0 to allow the Docker container shell to reach the host API
