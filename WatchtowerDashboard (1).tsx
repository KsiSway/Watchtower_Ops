...
export default function WatchtowerDashboard({
  ...
  const [recommendationTypeFilter, setRecommendationTypeFilter] = useState<'all' | 'duplicate' | 'near-duplicate' | 'orphaned' | 'large' | 'old'>('all');
  const [recommendationImpactFilter, setRecommendationImpactFilter] = useState<'all' | 'High' | 'Medium' | 'Low'>('all');
  ...
  useEffect(() => {
    ...
    if (!Array.isArray(history) || history.length === 0) {
      ...
      history = [
        { timestamp: new Date(now.getTime() - 6 * 3600 * 1000).toISOString(), score: Math.max(10, baseScore - 18) },
        { timestamp: new Date(now.getTime() - 5 * 3600 * 1000).toISOString(), score: Math.max(12, baseScore - 15) },
        { timestamp: new Date(now.getTime() - 4 * 3600 * 1000).toISOString(), score: Math.max(15, baseScore - 12) },
        { timestamp: new Date(now.getTime() - 3 * 3600 * 1000).toISOString(), score: Math.max(20, baseScore - 9) },
        { timestamp: new Date(now.getTime() - 2 * 3600 * 1000).toISOString(), score: Math.max(25, baseScore - 6) },
        { timestamp: new Date(now.getTime() - 1 * 3600 * 1000).toISOString(), score: Math.max(30, baseScore - 2) },
  ...
  const renderSevenSyncChart = () => {
    const lastSeven = scoreHistory.slice(-7);
    if (lastSeven.length === 0) return null;

    // Guard with fallback filling if there are less than 7 items
    const baseScore = stats.score;
    const paddedSeven = [...lastSeven];
    while (paddedSeven.length < 7) {
      const offsetIndex = 7 - paddedSeven.length;
      paddedSeven.unshift({
        timestamp: new Date(Date.now() - offsetIndex * 3600 * 1000).toISOString(),
        score: Math.max(10, baseScore - offsetIndex * 3)
      });
    }

    const data = paddedSeven.map((item, index) => {
    ...
    const initialScore = paddedSeven[0].score;
    const finalScore = paddedSeven[paddedSeven.length - 1].score;
    const diff = finalScore - initialScore;
    const isImproving = diff >= 0;
    const strokeColor = isImproving ? '#10b981' : '#f43f5e'; // emerald-500 vs rose-500
    const stopColor = isImproving ? '#10b981' : '#f43f5e';

    return (
      <div className="mt-5 border-t border-slate-850 pt-4 space-y-3" id="health-score-7sync-trend">
        <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-wider text-slate-400">
          <span className="flex items-center gap-1.5 font-bold">
            <TrendingUp className={`w-3.5 h-3.5 ${isImproving ? 'text-emerald-400 animate-pulse' : 'text-rose-400'}`} />
            7-Sync Health Trend
          </span>
          <span className={`px-2 py-0.5 rounded-md font-extrabold flex items-center gap-0.5 text-[9px] ${
            isImproving ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
          }`}>
            {isImproving ? '▲' : '▼'} {Math.abs(diff)} pts
          </span>
        </div>

        <div className="h-24 w-full" id="score-trend-chart-container">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 5, right: 8, left: -25, bottom: 5 }}>
              <defs>
                <linearGradient id="colorScoreTrendGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={stopColor} stopOpacity={0.25} />
                  <stop offset="95%" stopColor={stopColor} stopOpacity={0.0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" opacity={0.15} horizontal={true} vertical={false} />
              <XAxis 
                dataKey="name" 
                stroke="#475569" 
                fontSize={8}
              ...
              <Tooltip
                ...
                labelStyle={{ color: '#22d3ee', fontWeight: 'bold', fontSize: '8px', marginBottom: '2px' }}
              />
              <Area
                type="monotone"
                dataKey="score"
                ...
                strokeWidth={2}
                fillOpacity={1}
                fill="url(#colorScoreTrendGradient)"
                dot={{ r: 3, stroke: strokeColor, strokeWidth: 1.5, fill: '#090d16' }}
                activeDot={{ r: 5, stroke: '#22d3ee', strokeWidth: 2, fill: '#090d16' }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>

        <div className="flex items-center justify-between text-[8px] text-slate-500 font-mono tracking-wider">
          <span className="bg-slate-900/50 border border-slate-800/30 px-1.5 py-0.5 rounded">START: {initialScore} PTS</span>
          <span className="bg-slate-900/50 border border-slate-800/30 px-1.5 py-0.5 rounded">END: {finalScore} PTS</span>
        </div>
      </div>
    );
  };
  ...
  const downloadJSON = () => {
    ...
    if (exportOptions.includeRecommendations) {
      reportData.cleanupRecommendations = recommendations.map(rec => ({
        id: rec.id,
    ...
    if (exportOptions.includeCatalog) {
      reportData.fileCatalog = files.map(f => ({
        id: f.id,
        name: f.name,
        mimeType: f.mimeType,
        size: f.size,
    ...
    a.href = url;
    a.download = `watchtower-compliance-report-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const downloadCSV = () => {
    ...
    if (exportOptions.includeRecommendations) {
      ...
      csvContent += "ID,File Name,Type,Reason,Size (bytes),Size (formatted),Impact,Suggested Action\n";
      recommendations.forEach(rec => {
        const escapeCsv = (str: string) => `"${str.replace(/"/g, '""')}"`;
        csvContent += `${rec.id},${escapeCsv(rec.fileName)},${rec.type},${escapeCsv(rec.reason)},${rec.size || 0},${formatSize(rec.size)},${rec.impact || 'N/A'},${escapeCsv(rec.suggestedAction || '')}\n`;
      });
      csvContent += "\n";
    }
    ...
    if (exportOptions.includeCatalog) {
      ...
      csvContent += "ID,File Name,Mime Type,Size (bytes),Size (formatted),Last Modified,Tags\n";
      files.forEach(f => {
        const escapeCsv = (str: string) => `"${str.replace(/"/g, '""')}"`;
        const tagsStr = (f.tags || []).join(';');
        csvContent += `${f.id},${escapeCsv(f.name)},${escapeCsv(f.mimeType)},${f.size || 0},${formatSize(f.size)},${f.modifiedTime},${escapeCsv(tagsStr)}\n`;
      });
    }
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `watchtower-compliance-report-${new Date().toISOString().split('T')[0]}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-8" id="watchtower-dashboard-view">
      ...
      {/* Workspace Compliance & Health Exporter Card */}
      <div className="bg-[#090d16]/70 border border-slate-800/80 backdrop-blur-md rounded-3xl p-6 sm:p-8 shadow-lg space-y-6" id="compliance-exporter-card">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/60 pb-5">
          <div>
            <span className="text-[10px] uppercase tracking-widest text-emerald-400 font-bold block mb-1">Compliance Suite</span>
            <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
              <Award className="w-5 h-5 text-emerald-400" />
              Workspace Compliance & Health Exporter
            </h3>
            <p className="text-xs text-slate-400 mt-1 font-sans">
              Configure and package compliance audits, storage distribution charts, and security flags into a secure offline archive.
            </p>
          </div>
          <div className="flex items-center gap-2 bg-[#05070a] border border-slate-850 p-1 rounded-xl">
            <span className="text-[10px] uppercase font-bold text-slate-400 font-mono px-2 py-1">Report Grade:</span>
            <span className="px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-lg text-xs font-mono font-extrabold select-none">
              {stats.score >= 80 ? 'A+' : stats.score >= 60 ? 'B' : 'C-'} Compliance
            </span>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Left: Customizers & Selectors */}
          <div className="space-y-4">
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-widest">Select Modules to Export</h4>
            
            <div className="space-y-2.5 bg-[#040609]/60 border border-slate-850 p-4 rounded-2xl">
              <label className="flex items-start gap-3.5 p-2 hover:bg-slate-900/40 rounded-xl transition-all cursor-pointer group select-none">
                <input
                  type="checkbox"
                  checked={exportOptions.includeSummary}
                  onChange={(e) => setExportOptions({ ...exportOptions, includeSummary: e.target.checked })}
                  className="mt-0.5 rounded border-slate-800 text-cyan-500 focus:ring-cyan-500/30 bg-[#05070a] cursor-pointer transition-all"
                />
                <div className="space-y-0.5">
                  <span className="text-xs font-bold text-slate-200 group-hover:text-cyan-400 transition-colors">Overview Stats & Hygiene Scores</span>
                  <p className="text-[10px] text-slate-500 font-mono">Includes health scoring, total storage capacity, and file type counts.</p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3.5 p-2 hover:bg-slate-900/40 rounded-xl transition-all cursor-pointer group select-none">
                ...
                />
                <div className="space-y-0.5">
                  <span className="text-xs font-bold text-slate-200 group-hover:text-cyan-400 transition-colors">AI Project Classifications</span>
                  <p className="text-[10px] text-slate-500 font-mono">Includes intelligence labels, thematic clusters, and category descriptions.</p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3.5 p-2 hover:bg-slate-900/40 rounded-xl transition-all cursor-pointer group select-none">
                ...
                />
                <div className="space-y-0.5">
                  <span className="text-xs font-bold text-slate-200 group-hover:text-cyan-400 transition-colors">Pending Security & Clutter Alerts</span>
                  <p className="text-[10px] text-slate-500 font-mono">Includes flagged duplicates, near-duplicates, high-risk items, and cleanup actions.</p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3.5 p-2 hover:bg-slate-900/40 rounded-xl transition-all cursor-pointer group select-none">
                <input
                  type="checkbox"
                  checked={exportOptions.includeCatalog}
                  onChange={(e) => setExportOptions({ ...exportOptions, includeCatalog: e.target.checked })}
                  className="mt-0.5 rounded border-slate-800 text-cyan-500 focus:ring-cyan-500/30 bg-[#05070a] cursor-pointer transition-all"
                />
                <div className="space-y-0.5">
                  <span className="text-xs font-bold text-slate-200 group-hover:text-cyan-400 transition-colors">Full Active Metadata Catalog</span>
                  <p className="text-[10px] text-slate-500 font-mono">Includes complete filenames, sizes, relative timestamps, and custom tags.</p>
                </div>
              </label>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              <button
                type="button"
                onClick={downloadJSON}
                disabled={!exportOptions.includeSummary && !exportOptions.includeCategories && !exportOptions.includeRecommendations && !exportOptions.includeCatalog}
                className="w-full sm:w-auto flex-1 flex items-center justify-center gap-2 px-5 py-3 bg-cyan-600/20 hover:bg-cyan-600/35 disabled:opacity-40 disabled:hover:bg-cyan-600/20 disabled:cursor-not-allowed border border-cyan-500 text-cyan-400 rounded-xl text-xs font-bold transition-all uppercase tracking-widest cursor-pointer select-none"
              ...
              <button
                ...
                onClick={downloadCSV}
                disabled={!exportOptions.includeSummary && !exportOptions.includeCategories && !exportOptions.includeRecommendations && !exportOptions.includeCatalog}
                className="w-full sm:w-auto flex-1 flex items-center justify-center gap-2 px-5 py-3 bg-emerald-600/20 hover:bg-emerald-600/35 disabled:opacity-40 disabled:hover:bg-emerald-600/20 disabled:cursor-not-allowed border border-emerald-500 text-emerald-400 rounded-xl text-xs font-bold transition-all uppercase tracking-widest cursor-pointer select-none"
          ...
          {/* Right: Quick Diagnostic Metrics & Dynamic Preview Toggle */}
          <div className="flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              <h4 className="text-xs font-bold text-slate-300 uppercase tracking-widest">Workspace Diagnostics</h4>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-[#05070a]/60 border border-slate-850 rounded-2xl flex flex-col justify-center">
                  <span className="text-[9px] uppercase tracking-wider text-slate-500 font-mono block">Duplicate File Ratio</span>
                  <span className="text-xl font-bold font-mono text-cyan-400 mt-1">
                    {Math.round((recommendations.filter(r => r.type === 'duplicate' || r.type === 'near-duplicate').length / Math.max(1, files.length)) * 100)}%
                  </span>
                </div>
                <div className="p-4 bg-[#05070a]/60 border border-slate-850 rounded-2xl flex flex-col justify-center">
                  <span className="text-[9px] uppercase tracking-wider text-slate-500 font-mono block">Unclassified Files</span>
                  <span className="text-xl font-bold font-mono text-amber-400 mt-1">
                    {files.filter(f => !f.tags || f.tags.length === 0).length} items
                  </span>
                </div>
                <div className="p-4 bg-[#05070a]/60 border border-slate-850 rounded-2xl flex flex-col justify-center">
                  <span className="text-[9px] uppercase tracking-wider text-slate-500 font-mono block">Average File Size</span>
                  <span className="text-xl font-bold font-mono text-emerald-400 mt-1">
                    {formatSize(Math.round(stats.totalSize / Math.max(1, stats.totalFiles)))}
                  </span>
                </div>
                <div className="p-4 bg-[#05070a]/60 border border-slate-850 rounded-2xl flex flex-col justify-center">
                  <span className="text-[9px] uppercase tracking-wider text-slate-500 font-mono block">Total Flagged Items</span>
            ...
            <div>
              <button
                ...
                className="w-full text-center py-2.5 border border-slate-800/60 hover:border-slate-700 hover:bg-slate-900/40 rounded-xl text-[10px] font-mono font-bold text-slate-400 hover:text-slate-200 transition-all cursor-pointer"
              >
                {showExportPreview ? '▲ Hide Export Schema Preview' : '▼ Show Export Schema Preview'}
              </button>
            </div>
          </div>
        </div>
        ...
        {showExportPreview && (
          <div className="bg-[#030508] border border-slate-850 rounded-2xl p-4 font-mono text-[10px] text-slate-400 overflow-x-auto max-h-48 scrollbar-thin select-all">
            <span className="text-[9px] uppercase tracking-widest text-emerald-400 font-bold block mb-2 font-sans">Active Live Export Schema Mockup:</span>
...
{JSON.stringify({
  ...
  ...(exportOptions.includeRecommendations && {
    cleanupRecommendations: recommendations.slice(0, 1).map(r => ({
      id: r.id,
      fileName: r.fileName,
      type: r.type,
      reason: r.reason,
  ...
  ...(exportOptions.includeCatalog && {
    fileCatalog: files.slice(0, 1).map(f => ({
      id: f.id,
      name: f.name,
      mimeType: f.mimeType,
      size: f.size,
      ...
      <div className="bg-[#090d16]/70 border border-slate-800/80 backdrop-blur-md rounded-3xl p-6 sm:p-8 shadow-lg" id="cleanup-recommendations-panel">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-slate-850 pb-5 mb-6 gap-4">
          <div>
            <div className="flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-cyan-400 animate-pulse" />
              <h3 className="text-lg font-bold text-slate-100 tracking-tight">AI Workspace Health & Deep Clean Console</h3>
            </div>
            <p className="text-xs text-slate-400 mt-1">
              Active heuristic audit for duplicate versions, unlinked/orphaned documents, and outdated storage weight
            </p>
          </div>
          
          <div className="flex flex-wrap items-center gap-3">
            <span className="px-3 py-1.5 bg-[#05070a] border border-slate-800 rounded-xl font-mono text-[11px] text-cyan-400 font-bold flex items-center gap-2">
              <Layers className="w-3.5 h-3.5 text-cyan-500" />
              Reclaimable: {formatSize(recommendations.reduce((sum, rec) => sum + (rec.size || 0), 0))}
            </span>
            <span className="px-3 py-1.5 bg-cyan-950/40 border border-cyan-800/40 text-cyan-300 font-bold text-xs rounded-xl flex items-center gap-1.5 uppercase tracking-wider">
        ...
        {/* Audit Filter Control Station */}
        <div className="bg-[#04060a]/90 border border-slate-850 rounded-2xl p-4 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-inner" id="audit-filter-station">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="text-slate-500 font-mono font-bold uppercase tracking-wider text-[10px] mr-1">Classification:</span>
            
            <button
              onClick={() => setRecommendationTypeFilter('all')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border cursor-pointer ${
                recommendationTypeFilter === 'all'
                  ...
                  : 'bg-[#020305] border-slate-800 text-slate-400 hover:text-slate-200'
              }`}
            >
              All Types
            </button>
            ...
            <button
              onClick={() => setRecommendationTypeFilter('near-duplicate')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border cursor-pointer flex items-center gap-1.5 ${
                recommendationTypeFilter === 'near-duplicate'
                  ? 'bg-purple-500/15 border-purple-500 text-purple-300 shadow-[0_0_8px_rgba(168,85,247,0.15)]'
                  : 'bg-[#020305] border-slate-800 text-slate-400 hover:text-purple-400 hover:border-purple-900/50'
              }`}
              title="Successive drafts or closely matching titles"
            >
              <Copy className="w-3 h-3 text-purple-400" />
              Draft Revisions ({recommendations.filter(r => r.type === 'near-duplicate').length})
            </button>

            <button
              onClick={() => setRecommendationTypeFilter('orphaned')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border cursor-pointer flex items-center gap-1.5 ${
                recommendationTypeFilter === 'orphaned'
                  ? 'bg-indigo-500/15 border-indigo-500 text-indigo-300 shadow-[0_0_8px_rgba(99,102,241,0.15)]'
                  : 'bg-[#020305] border-slate-800 text-slate-400 hover:text-indigo-400 hover:border-indigo-900/50'
              }`}
              title="Generic untitled / unlinked files"
            >
              <AlertCircle className="w-3 h-3 text-indigo-400" />
              Orphaned Files ({recommendations.filter(r => r.type === 'orphaned').length})
            </button>

            <button
              onClick={() => setRecommendationTypeFilter('large')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border cursor-pointer flex items-center gap-1.5 ${
                recommendationTypeFilter === 'large'
                  ? 'bg-rose-500/15 border-rose-500 text-rose-300 shadow-[0_0_8px_rgba(239,68,68,0.15)]'
                  : 'bg-[#020305] border-slate-800 text-slate-400 hover:text-rose-400 hover:border-rose-900/50'
              }`}
              title="Outdated storage hogs over 5MB"
            >
              <HardDrive className="w-3 h-3 text-rose-400" />
              Large Obsolete ({recommendations.filter(r => r.type === 'large').length})
            </button>

            <button
              onClick={() => setRecommendationTypeFilter('duplicate')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border cursor-pointer flex items-center gap-1.5 ${
                recommendationTypeFilter === 'duplicate'
                  ? 'bg-amber-500/15 border-amber-500 text-amber-300 shadow-[0_0_8px_rgba(245,158,11,0.15)]'
                  : 'bg-[#020305] border-slate-800 text-slate-400 hover:text-amber-400 hover:border-amber-900/50'
              }`}
              title="Direct file size & name duplicates"
            >
              <Layers className="w-3 h-3 text-amber-400" />
              Exact Clones ({recommendations.filter(r => r.type === 'duplicate').length})
            </button>
            
            <button
              onClick={() => setRecommendationTypeFilter('old')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border cursor-pointer flex items-center gap-1.5 ${
                recommendationTypeFilter === 'old'
                  ? 'bg-emerald-500/15 border-emerald-500 text-emerald-300 shadow-[0_0_8px_rgba(16,185,129,0.15)]'
                  : 'bg-[#020305] border-slate-800 text-slate-400 hover:text-emerald-400 hover:border-emerald-900/50'
              }`}
              title="Unmodified files over 180 days old"
            >
              <CheckCircle className="w-3 h-3 text-emerald-400" />
              Stale Files ({recommendations.filter(r => r.type === 'old').length})
            </button>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-[10px] text-slate-500 font-mono font-bold uppercase">Impact:</span>
            <select
              value={recommendationImpactFilter}
              onChange={(e) => setRecommendationImpactFilter(e.target.value as any)}
              className="bg-[#020305] border border-slate-800 rounded-lg text-xs py-1.5 px-2.5 font-bold text-slate-300 focus:outline-hidden"
              id="recommendation-impact-selector"
            >
        ...
        {/* Audit Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in" id="recommendations-container">
          {recommendations.filter(rec => {
            const matchesType = recommendationTypeFilter === 'all' || rec.type === recommendationTypeFilter;
            const matchesImpact = recommendationImpactFilter === 'all' || (rec.impact || 'Medium') === recommendationImpactFilter;
            ...
            recommendations.filter(rec => {
              ...
              const isNearDuplicate = rec.type === 'near-duplicate';
              const isOrphaned = rec.type === 'orphaned';
              const isLarge = rec.type === 'large';

              return (
                <div
                  key={rec.id}
                  className="bg-[#05070a]/70 border border-slate-850 hover:border-slate-800 rounded-2xl p-5 flex flex-col justify-between gap-4 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 relative group"
                  id={`rec-item-${rec.id}`}
                >
                  <div>
                    {/* Header: Class, Impact and Size */}
                    <div className="flex items-center justify-between gap-2 mb-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className={`w-1.5 h-1.5 rounded-full ${
                          rec.impact === 'High' ? 'bg-rose-500 animate-pulse' : rec.impact === 'Medium' ? 'bg-amber-500' : 'bg-cyan-500'
                        }`} />
                        <span className="text-[10px] text-slate-400 font-mono font-bold uppercase">{rec.impact || 'Medium'}</span>
                      </div>
                    </div>

                    {/* File Name & Detail */}
                    <h4 className="text-xs font-bold text-slate-200 line-clamp-1 group-hover:text-cyan-400 transition-colors" title={rec.fileName}>
                      {rec.fileName}
                    </h4>
                    
                    {/* Reclaimable Size Indicator */}
                    {rec.size ? (
                      <div className="mt-1.5 text-[10px] font-mono text-emerald-400 font-bold flex items-center gap-1">
                    ...
                    {/* AI / Heuristic Reason */}
                    <p className="text-[11px] text-slate-400 mt-2.5 leading-relaxed bg-[#020305]/40 border border-slate-900/50 p-2.5 rounded-xl min-h-[55px]">
                      {rec.reason}
                    </p>

                    {/* Suggested Smart Action */}
                    <div className="mt-3 flex items-start gap-1.5 text-[11px] text-cyan-400 bg-cyan-950/15 border border-cyan-900/30 p-2 rounded-xl">
                      <Sparkles className="w-3.5 h-3.5 text-cyan-400 shrink-0 mt-0.5 animate-pulse" />
                      <div>
                        <span className="font-bold text-[10px] font-mono uppercase text-cyan-300">Action Plan:</span>
                        <p className="text-slate-300 mt-0.5">{rec.suggestedAction || 'Review file detail for consolidation'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-850">
                    <button
                      ...
                      className="flex-1 px-3 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 rounded-xl text-[11px] font-bold transition-all uppercase tracking-wider cursor-pointer font-mono"
                    >
                      Review File
                    </button>
                    <button
                      onClick={() => onCleanUp(rec.id, rec.fileName)}
                      className="p-2 bg-rose-950/20 hover:bg-rose-950/45 text-rose-400 hover:text-rose-300 rounded-xl transition-all border border-rose-900/40 cursor-pointer"
                      title="Apply recommended cleanup"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
            ...
            <div className="col-span-3 bg-slate-950/30 border border-slate-850 text-slate-400 p-12 rounded-2xl text-center flex flex-col items-center justify-center">
              <CheckCircle className="w-8 h-8 text-cyan-400 mb-3 opacity-60" />
              <h4 className="text-sm font-bold text-slate-300">No Audited Items Found</h4>
              <p className="text-xs text-slate-500 mt-1 max-w-sm">
                No files match the currently selected combination of classification filters and impact levels.
              </p>
            </div>
          )}
        </div>
