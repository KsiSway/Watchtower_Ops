...
import { AnimatePresence } from 'motion/react'; // 'motion' default is not used
import {
...
export default function App() {
  ...
  const runWorkspaceSync = async (token: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Google Drive API returned an error');
      }
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      addLog('Auto-initiating synchronization scan: activating personal intelligence tunnels...', 'neural');
      triggerForceRefresh();
  ...
  return (
    <div className={`min-h-screen bg-[#05070a] text-slate-200 font-sans relative overflow-hidden ${theme === 'terminal-light' ? 'theme-terminal-light' : ''}`} id="workspace-watchtower-app">
      ...
      <div className="relative z-20 bg-[#090d16] border-b border-cyan-500/20 text-cyan-400 py-2.5 px-4 text-center text-[10px] uppercase tracking-widest font-bold flex items-center justify-center gap-2" id="top-notification-banner">
        ...
        <span>Watchtower Intelligence initialized with <strong className="text-amber-300">gemini-3.5-flash</strong> // secure shell active</span>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Smart file hygiene & summaries powered by personal intelligence</p>
            </div>
          </div>
