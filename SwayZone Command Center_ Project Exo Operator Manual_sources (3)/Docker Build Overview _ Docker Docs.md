---
sourceFile: "Docker Build Overview | Docker Docs"
exportedBy: "Kortex"
exportDate: "2026-05-15T09:23:16.286Z"
---

# Docker Build Overview | Docker Docs

660e00fb-4d36-4a2e-95d6-6ecf2456d4aa

Docker Build Overview | Docker Docs

4b8a3b7c-1565-49ba-b693-a7895e88b317
...
https://docs.docker.com/build/concepts/overview/#buildx
...
Buildx and BuildKit are both installed with Docker Desktop and Docker Engine out-of-the-box. When you invoke the

docker build

command, you're using Buildx to run a build using the default BuildKit bundled with Docker.

https://docs.docker.com/build/concepts/overview/#buildx

Buildx is the CLI tool that you use to run builds. The

docker build

command is a wrapper around Buildx. When you invoke

docker build

, Buildx interprets the build options and sends a build request to the BuildKit backend.
...
Docker Buildx is installed by default with Docker Desktop. You can also build the CLI plugin from source, or grab a binary from the GitHub repository and install it manually. See
...
invokes Buildx under the hood, there are subtle differences between this command and the canonical
...
command. Buildx interprets your build command and sends a build request to the BuildKit backend. The build request includes:
...
https://github.com/docker/docs/edit/main/content/manuals/build/concepts/overview.md

Request changes

https://github.com/docker/docs/issues/new?template=doc\_issue.yml&location=https%3a%2f%2fdocs.docker.com%2fbuild%2fconcepts%2foverview%2f&labels=status%2Ftriage
