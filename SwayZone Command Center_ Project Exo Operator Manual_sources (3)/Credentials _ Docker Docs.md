---
sourceFile: "Credentials | Docker Docs"
exportedBy: "Kortex"
exportDate: "2026-05-15T09:23:16.213Z"
---

# Credentials | Docker Docs

ee5e3e0d-04d9-48cd-828b-6686ea79e898

Credentials | Docker Docs

57523a9e-c49d-4701-8995-7f56a10bf6c3

https://docs.docker.com/ai/sandboxes/security/credentials/#stored-secrets
...
https://docs.docker.com/ai/sandboxes/security/credentials/#how-credential-injection-works
...
https://docs.docker.com/ai/sandboxes/security/credentials/#store-a-secret

Built-in services

https://docs.docker.com/ai/sandboxes/security/credentials/#built-in-services

Services declared by kits

https://docs.docker.com/ai/sandboxes/security/credentials/#services-declared-by-kits

List and remove secrets

https://docs.docker.com/ai/sandboxes/security/credentials/#list-and-remove-secrets

GitHub token

https://docs.docker.com/ai/sandboxes/security/credentials/#github-token

https://docs.docker.com/ai/sandboxes/security/credentials/#ssh-agent

Custom secrets

https://docs.docker.com/ai/sandboxes/security/credentials/#custom-secrets

Environment variables

https://docs.docker.com/ai/sandboxes/security/credentials/#environment-variables

Best practices

https://docs.docker.com/ai/sandboxes/security/credentials/#best-practices

Custom templates and placeholder values

https://docs.docker.com/ai/sandboxes/security/credentials/#custom-templates-and-placeholder-values

Availability: Early Access

Most agents need an API key for their model provider. An HTTP/HTTPS proxy on your host intercepts outbound requests from the sandbox, looks up the matching credential on the host, and overwrites the auth header before forwarding. The real credential stays on the host; the sandbox sees only a sentinel value. For the security model behind this, see

Credential isolation

https://docs.docker.com/ai/sandboxes/security/isolation/#credential-isolation
...
Stored secrets, keyed on a service identifier: built-in agents declare service identifiers (

, etc.) in their kit specs; custom kits can declare their own.

sbx secret set

stores a value keyed on that identifier. When a sandboxed request matches a service's domain, the proxy reads the stored value and writes the configured header. Inside the sandbox, the environment variable holds a sentinel like

proxy-managed

, so SDKs that read the variable see something non-empty without seeing the real secret. See
...
stores a value alongside a target domain, an environment variable name, and an optional placeholder. The sandbox sees the placeholder; the proxy substitutes it with the real value anywhere it appears in outbound traffic to that domain. Use this when the service-identifier model doesn't fit — for example, when the agent validates the variable format at boot, or when the credential lands in a request body. See

Custom secrets

https://docs.docker.com/ai/sandboxes/security/credentials/#custom-secrets

Host shell environment variables: as a fallback, the proxy reads from your shell environment. Useful for one-off testing or development; stored secrets are preferred because shell environment variables are plaintext and visible to other processes running as your user. See

Environment variables

https://docs.docker.com/ai/sandboxes/security/credentials/#environment-variables

If both a stored secret and a host environment variable are set for the same service, the stored secret takes precedence. For multi-provider agents (OpenCode, Docker Agent), the proxy selects credentials based on the API endpoint being called. See individual
...
stores credentials in your OS keychain, keyed on a service identifier. Built-in agents declare a fixed set of services. Custom kits can declare their own. The same
...
This prompts you for the secret value interactively. The

flag stores the secret globally so it's available to all sandboxes. To scope a secret to a specific sandbox instead:

$ sbx secret set my-sandbox openai

A sandbox-scoped secret takes effect immediately, even if the sandbox is running. A global secret (

) only applies when a sandbox is created. If you set or change a global secret while a sandbox is running, recreate the sandbox for the new value to take effect.

#### You can also pipe in a value for non-interactive use:

$ echo "$ANTHROPIC\_API\_KEY" | sbx secret set -g anthropic
...
, the proxy uses it the same way it would use the corresponding environment variable. You don't need to set both.

Services declared by kits

https://docs.docker.com/ai/sandboxes/security/credentials/#services-declared-by-kits

Custom kits can declare their own service identifiers in

— they're not limited to the table above. To provide a credential for a kit-declared service, run

sbx secret set

with the same identifier the kit declares under

credentials.sources

$ sbx secret set -g my-service

There's no separate registration step; the keychain entry is keyed on the identifier the kit already uses. See
...
(global)   github    gho\_GCaw4o\*\*\*\*...\*\*\*\*43qy

#### Remove a secret:

$ sbx secret rm -g github

deletes all stored secrets along with all sandbox state. You'll need to re-add your secrets after a reset.

GitHub token

https://docs.docker.com/ai/sandboxes/security/credentials/#github-token

service gives the agent access to the

#### CLI inside the sandbox. Pass your existing GitHub CLI token:
...
If your host has an SSH agent and

SSH\_AUTH\_SOCK

is set, Docker Sandboxes forwards the agent into the sandbox and sets

SSH\_AUTH\_SOCK

there. The private keys stay on your host. Processes inside the sandbox can request signatures from the forwarded agent, but they can't read or copy the private key.

Use SSH agent forwarding for Git operations over SSH and SSH-based commit signing. The signing key must be loaded in the host SSH agent for sandboxed commit signing to work. Outbound SSH connections are still subject to sandbox network policy. For details, see

Signed commits

https://docs.docker.com/ai/sandboxes/usage/#signed-commits

Custom secrets

https://docs.docker.com/ai/sandboxes/security/credentials/#custom-secrets

Custom secrets are experimental. The

command is hidden from

, and behavior, flags, and the placeholder format may change.

For credentials that don't fit the service-identifier model — for example, when an agent validates the environment variable format at boot, or when the credential lands in a request body rather than a header — use

sbx secret set-custom

. The secret is keyed on a target domain, an environment variable name, and an optional placeholder string, instead of a service identifier.
...
records it in your shell history and exposes it to other processes running as your user. Avoid pasting real credentials inline — read the value from a variable that's already in your environment, and clear shell history if a real secret was passed on the command line.

Inside the sandbox,

is set to a generated placeholder (for example,

sbx-cs-<rand>

). When a sandboxed process sends a request to

api.example.com

and the placeholder appears anywhere in the request, the proxy replaces it with the real value. The agent never sees the real secret.

service-based flow

https://docs.docker.com/ai/sandboxes/security/credentials/#stored-secrets

whenever it's an option — the kit handles the wiring; you only provide the value.
...
$ export ANTHROPIC\_API\_KEY=sk-ant-api03-xxxxx
$ sbx run claude
...
for the variable names each agent expects.

These environment variables are set on your host, not inside the sandbox. Sandbox agents are pre-configured to use credentials managed by the host-side proxy. For custom environment variables not tied to a
...
over environment variables. The OS keychain encrypts credentials at rest and controls access, while environment variables are plaintext in your shell.

Don't set API keys manually inside the sandbox. Sandbox agents are pre-configured to use proxy-managed credentials.

For Claude Code and Codex, OAuth is another secure option: the flow runs on the host, so the token is never exposed inside the sandbox. For Claude Code, use

inside the agent. For Codex, run
...
When building custom templates or installing agents manually in a shell sandbox, some agents require environment variables like
...
) if needed. The proxy injects actual credentials regardless of the environment variable value.

Edit this page

https://github.com/docker/docs/edit/main/content/manuals/ai/sandboxes/security/credentials.md

Request changes

https://github.com/docker/docs/issues/new?template=doc\_issue.yml&location=https%3a%2f%2fdocs.docker.com%2fai%2fsandboxes%2fsecurity%2fcredentials%2f&labels=status%2Ftriage
