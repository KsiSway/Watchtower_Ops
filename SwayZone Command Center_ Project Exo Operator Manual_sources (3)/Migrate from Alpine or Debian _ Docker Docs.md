---
sourceFile: "Migrate from Alpine or Debian | Docker Docs"
exportedBy: "Kortex"
exportDate: "2026-05-15T09:23:16.659Z"
---

# Migrate from Alpine or Debian | Docker Docs

e8542aaa-721d-4593-bc30-0e3db3716bf6

Migrate from Alpine or Debian | Docker Docs

ce2d796d-7f9c-4dc3-80d1-089970c59b49

https://docs.docker.com/guides/dhi-from-doi/
...
https://docs.docker.com/guides/dhi-from-doi/#key-differences

Migration steps

https://docs.docker.com/guides/dhi-from-doi/#migration-steps

Step 1: Update the base image in your Dockerfile

https://docs.docker.com/guides/dhi-from-doi/#step-1-update-the-base-image-in-your-dockerfile

Step 2: Update the runtime image in your Dockerfile

https://docs.docker.com/guides/dhi-from-doi/#step-2-update-the-runtime-image-in-your-dockerfile

Language-specific examples

https://docs.docker.com/guides/dhi-from-doi/#language-specific-examples

Docker Hardened Images (DHI) come in both

Alpine-based and Debian-based variants

https://docs.docker.com/dhi/explore/available/

. In many cases, migrating from another image based on these distributions is as simple as changing the base image in your Dockerfile.
...
Package managers generally available in all images.

Package managers generally only available in images with a

tag. Runtime images don't contain package managers. Use multi-stage builds and copy necessary artifacts from the build stage to the runtime stage.

Non-root user

Usually runs as root by default

Runtime variants run as the nonroot user by default. Ensure that necessary files and directories are accessible to the nonroot user.

Multi-stage build

Recommended. Use images with a

tags for build stages and non-dev images for runtime.

TLS certificates

May need to be installed

Contain standard TLS certificates by default. There is no need to install TLS certificates.

Can bind to privileged ports (below 1024) when running as root

Run as a nonroot user by default. Applications can't bind to privileged ports (below 1024) when running in Kubernetes or in Docker Engine versions older than 20.10. Configure your application to listen on port 1025 or higher inside the container.

Entry point

Varies by image

May have different entry points than Docker Official Images. Inspect entry points and update your Dockerfile if necessary.

Shell generally available in all images

Runtime images don't contain a shell. Use

images in build stages to run shell commands and then copy artifacts to the runtime stage.
...
Update the base image in your application's Dockerfile to a hardened image. This is typically going to be an image tagged as
...
before you can pull Docker Hardened Images. Use your Docker ID credentials (the same username and password you use for Docker Hub). If you don't have a Docker account,
...
Note that DHI does not have a

tag in order to promote best practices around image versioning. Ensure that you specify the appropriate version tag for your image. To find the right tag, explore the available tags in the

DHI Catalog

https://hub.docker.com/hardened-images/catalog/

. In addition, the distribution base is specified in the tag (for example,

-alpine3.22

), so be sure to select the correct variant for your application.

Step 2: Update the runtime image in your Dockerfile

https://docs.docker.com/guides/dhi-from-doi/#step-2-update-the-runtime-image-in-your-dockerfile

Multi-stage builds are recommended to keep your final image minimal and secure. Single-stage builds are supported, but they include the full

image and therefore result in a larger image with a broader attack surface.
...
. All stages in your Dockerfile should use a hardened image. While intermediary stages will typically use images tagged as
...
After updating your Dockerfile, build and test your application. If you encounter issues, see the
...
https://github.com/docker/docs/edit/main/content/manuals/dhi/migration/migrate-from-doi.md

Request changes

https://github.com/docker/docs/issues/new?template=doc\_issue.yml&location=https%3a%2f%2fdocs.docker.com%2fdhi%2fmigration%2fmigrate-from-doi%2f&labels=status%2Ftriage
