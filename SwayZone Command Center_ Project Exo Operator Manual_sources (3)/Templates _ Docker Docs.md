---
sourceFile: "Templates | Docker Docs"
exportedBy: "Kortex"
exportDate: "2026-05-15T09:23:16.741Z"
---

# Templates | Docker Docs

9078581f-d300-48db-a8ae-ffd9ae3516a4

Templates | Docker Docs

60885811-eeff-4238-a0b9-96aeb8590072
...
https://docs.docker.com/ai/sandboxes/customize/templates/#custom-templates

Base images

https://docs.docker.com/ai/sandboxes/customize/templates/#base-images
...
https://docs.docker.com/ai/sandboxes/customize/templates/#template-caching

Saving a sandbox as a template

https://docs.docker.com/ai/sandboxes/customize/templates/#saving-a-sandbox-as-a-template

Save and reuse

https://docs.docker.com/ai/sandboxes/customize/templates/#save-and-reuse

List and remove templates

https://docs.docker.com/ai/sandboxes/customize/templates/#list-and-remove-templates

Export and import

https://docs.docker.com/ai/sandboxes/customize/templates/#export-and-import

Limitations

https://docs.docker.com/ai/sandboxes/customize/templates/#limitations
...
Custom templates customize an existing agent's environment — they don't create new agent runtimes. The agent that launches inside the sandbox is determined by the base image variant you extend and the agent you specify in the

command, not by binaries installed in the template. To define a new agent from scratch, see

https://docs.docker.com/ai/sandboxes/customize/kits/#defining-an-agent
...
. They are based on Ubuntu and run as a non-root

user with sudo access. Most variants include Git, Docker CLI, and common development tools like Node.js, Python, Go, and Java.
...
) that includes a full Docker Engine running inside the sandbox — no local Docker daemon required. When you pick a built-in agent without specifying a custom template,

template variants by default.

The agent containers created from the

templates run in privileged mode inside the microVM (not on your host), with a dedicated block volume at

/var/lib/docker

starts automatically inside the sandbox. The block volume defaults to 50 GB and uses a sparse file, so it only consumes disk space as Docker writes to it.
...
Use the non-Docker variant if you don't need to build or run containers inside the sandbox and want a lighter, non-privileged environment. Specify it explicitly with
...
Write a Dockerfile that extends one of the base images. Pick the variant that matches the agent you plan to run. For example, extend

claude-code

to customize a Claude Code environment, or

to customize an OpenAI Codex environment.
...
before installing user-level tools. Tools that install into the home directory, such as

, must run as

— otherwise they install under

and aren't available in the sandbox.
...
session to pull private images from Docker Hub. Other registries (such as GitHub Container Registry, ECR, or a self-hosted registry like Nexus) are pulled anonymously, so private images on those registries fail to pull.

For locally-built images or private images on registries that

can't authenticate against, save the image to a tar and load it directly into the sandbox runtime instead of pulling from a registry:
...
imports the tar into the sandbox runtime's image store, so the image doesn't need to be reachable from a registry at sandbox creation time.

Unless you use the permissive

network policy, you may also need to allow-list any domains that your custom tools depend on:
...
does not automatically resolve the Docker Hub domain (
...
Template images are cached locally. The first use pulls from the registry; subsequent sandboxes reuse the cache. Cached images persist across sandbox creation and deletion, and are cleared when you run
...
The image is stored in the sandbox runtime's local image store. Create a new sandbox from it with the
...
Agent configuration files are always recreated when a sandbox is created. Changes to user-level agent configuration files, such as

/home/agent/.claude/settings.json

/home/agent/.claude.json

, do not persist in saved templates.

If the saved template was built for a different agent than the one you specify in

, you get a warning. For example, saving a Claude sandbox and running it with
...
https://github.com/docker/docs/edit/main/content/manuals/ai/sandboxes/customize/templates.md

Request changes

https://github.com/docker/docs/issues/new?template=doc\_issue.yml&location=https%3a%2f%2fdocs.docker.com%2fai%2fsandboxes%2fcustomize%2ftemplates%2f&labels=status%2Ftriage
